...
heap_t *heap_insert(heap_t **root, int value)
{
	...
	int size, lve, subtrctn, btt, level, tmp;
	...
	lve = size;
	for (level = 0, subtrctn = 1; lve >= subtrctn; subtrctn *= 2, level++)
		lve -= subtrctn;
	/* subtract all nodes except for bottom-most level */

	for (btt = 1 << (level - 1); btt != 1; btt >>= 1)
		tree = lve & btt ? tree->right : tree->left;
	/*
	 * Traverse tree to first empty slot based on the binary
	 * representation of the number of lve.
	 * Example -
	 * If there are 12 nodes in a complete tree, there are 5 lve on
	...
	lve & 1 ? (tree->right = new) : (tree->left = new);
