		...
		avl_t **new, int value)
{
	...
	befactor = balance(*tree);
	if (befactor > 1 && (*tree)->left->n > value)
		*tree = binary_tree_rotate_right(*tree);
	else if (befactor < -1 && (*tree)->right->n < value)
		*tree = binary_tree_rotate_left(*tree);
	else if (befactor > 1 && (*tree)->left->n < value)
	{
...
avl_t *avl_insert(avl_t **tree, int value)
{
	...
	avl_insert_recursive(tree, *tree, &another, value);
	return (another);
}
